...
const envNames = Object.keys(env)
  .filter(function (x) {
    return x.indexOf('APPSETTING_') === 0;
  })
  .filter(function (x) {
    return x.indexOf('APPSETTING_WEBSITE_') < 0;
  });
const appSettings = envNames.reduce(function (cur, key) {
  return { ...cur, [key.substring(11)]: env[key] };
}, {});
...
function updateSettingsFile(appSettingPath, envPath, settings) {
  if (isNodeJSProject()) {
    ...
    fs.writeFileSync(appSettingPath, JSON.stringify(settings, null, 2), {
      encoding: 'utf-8',
    });
  }
}
